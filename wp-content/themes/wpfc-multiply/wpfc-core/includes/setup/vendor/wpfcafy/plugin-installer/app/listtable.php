 ...
 * @since 1.0.0
 */
class Wpfcafy_PluginInstaller_ListTable extends WP_Plugin_Install_List_Table {
	...
	public function query_dotorg() {
		$plugins = wpfcafy_plugininstaller_get_option( 'plugins' );
		...
		if ( false === ( $items = get_transient( $key ) ) ) {
			...
			foreach ( $plugins as $plugin ) {
				$wp_plugin = plugins_api( 'plugin_information', wp_parse_args( array( 'slug' => $plugin ), $args ) );

				if ( ! is_wp_error( $wp_plugin ) ) {
					if ( $wp_plugin->slug !== $plugin ) {
						$wp_plugin->slug = $plugin;
					}
			...
			set_transient( 'wpfc_3rd_party_plugins_data', $key, DAY_IN_SECONDS );
		}
	...
	public function display() {
        ...
        <form id="plugin-filter" class="wpfcafy-plugininstaller" method="post">
