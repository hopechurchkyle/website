...
try {
    ...
    if (!isset($gantry['theme'])) {
        if (file_exists($gantry['theme.path'] . '/wpfc-core/includes/theme.php')) {
            include_once $gantry['theme.path'] . '/wpfc-core/includes/theme.php';
        }
        
        if (file_exists($gantry['theme.path'] . '/includes/theme.php')) {
    ...
    add_filter('template_include', function () use ($e) {
        if (is_customize_preview() && !class_exists('Timber')) {
            _e('Timber library plugin not found. ', 'wpfc-core');
        }
        _e('Theme cannot be used. For more information, please see the notice in administration.', 'wpfc-core');
        die();
    });
