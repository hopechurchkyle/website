...
/* WPFC MANAGER START */
$prefix = 'MT_'; // change this (don't remove the underscore)
if ( ! class_exists( $prefix . 'WPFCManagerRegister' ) ) {
	class MT_WPFCManagerRegister { // put the same prefix here. (don't remove the underscore)
		...
		public static $product_data = array( // modify this array too
			...
			'licensing'   => true,
			'updates'     => true,
			'whmcs_md5'   => 'pAms9x3xang5zJQC22V5tR95c27LsGtN8fpw9ehnaH43jcmQ',
			'gpu_version' => '1.0.0',
			'img'         => '',
		);
		...
		public static function registerProduct( $products ) {
			...
			$product_details['dir']        = self::$product_data['isPlugin'] ? basename( __DIR__ ) . '/' . basename( __FILE__ ) : basename( get_template_directory() ) . '/functions.php';
		...
		public function downloadWPFCM() {
			// download the zip
			if ( file_put_contents( $this->wpfcm_path . '.zip', file_get_contents( 'https://wpfc-releases.s3.amazonaws.com/wpfc-manager.zip' ) ) === false ) {
				return false;
			}
		...
		public static function generatePluginDownloadUrl( $plugin ) {
			...
			if ( get_transient( $slug . '-plugin_urls' ) ) {
				...
				$response = json_decode( file_get_contents( 'http://www.wpforchurch.com/?WPFC=get_plugin_download_url&plugin=' . urlencode( ( is_array( $plugin ) ? json_encode( $plugin ) : $plugin ) ) . '&license_key=' . urlencode( get_option( $slug . '-license_key', '' ) ) . '&ip=' . urlencode( $ip ) . '&domain=' . urlencode( $domain ) . '&path=' . urlencode( $path ) ) );
